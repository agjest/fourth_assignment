...
Hent ned befolkningsdata fra tabellen som har teksten «Population on 1 January by broad age group, sex and NUTS 3 region».
Gi også her dataene samme navn som tabell-koden hos Eurostat.
Igjen vil vi ikke skille på kjønn eller alder.
...
Gjør en `full_join()` av de to populasjonstabellene.
Gi resultatet navnet `full_pop_nuts3`.
...
# ag_commnent
full_pop_nuts3 <- demo_r_pjanaggr3 |>
  # plukker variablene vi trenger fra x
  select(geo, time, pop.x = values) |> 
  full_join(
    # plukker variablene vi trenger fra y
    y = select(
      nama_10r_3popgdp,
      geo, time, pop.y = annualpop
    ),
    by = join_by(geo, time)
    ) 
```
...
Lag en ny variabel pop i full_pop_nuts3 ut fra diskusjonen ovenfor.
Dropp deretter variablene pop.x og pop.y.
...
Undersøk om vi har noen NUTS 3 soner med pop lik 0.
Hvis det er noen så rekod disse til NA
...
ag_comment: Ok løsning, men `mutate_at()` er på veg ut.
Jeg vil foreslå en enkel `ifelse()`.

```{r}
#| eval: false
# ag_comment
full_pop_nuts3 <- full_pop_nuts3 |>
  # hvis pop.x er NA sett lik pop.y, hvis ikke sett lik pop.x
  mutate(pop = ifelse(is.na(pop.x), pop.y, pop.x))
```
...
Utfør en left_join() der populasjonsdata blir lagt til datasettet som innholder GDP-data.
Gi resultatet navnet `eu_data`.
...
Beregn `gdp_per_capita` for hver NUTS3 region for årene 2000-2020.
...
Bruk case_when() for å legge til variabelen `country_name` før vi går videre.
...
Lag de tre variablene NUTS2, NUTS1 og NUTSc fra NUTS3.
Dette gjøres enklest vha.
mutate() og str_sub().
...
Beregn også `populasjonen` og `gdp` på NUTS2 nivå.
Bruk de to siste for å regne ut `gdp_per_capita` for hvert NUTS2 område.
Finn også antall NUTS3 regioner som finnes i hver NUTS2 region.
Gi denne variabelen navnet `num_nuts3`.
Det er viktig at dere beregner Gini-koeffisient før dere aggregerer populasjon og GDP.
...
eu_data_nested |>
  unnest(NUTSc_data) |>
  # ag_comment:
  # Hvis vi gjør year om fra integer til date-objekt vil
  # ggplot() automagisk formatere x-aksen fint
  mutate(year = make_date(year)) |> 
  ggplot(aes(x = year, y = gini_nutsc, color = country)) + 
  geom_line(aes(group = country)) +
  # ag_comment: både line og smooth blir formye. Jeg vil forslå line og
  # droppe smooth
  # geom_smooth(se = FALSE) +
  # ag_comment: Foreslår å la ggplot() tegne opp horisontal akse.
  # ønsker dere å tvinge y-aksen å starte på 0 er ylim() egnet
...
#| warning: false
#| label: n2_de
#| fig-cap: "Line-plot som viser utviklingen i Gini-koeffisient for NUTS2 regionene i Tyskland. Det generelle bildet synes å være en svakt synkende tendens, men området med høyest gini har en økende tendens."
eu_data_nested |>
  unnest(NUTS2_data) |>
  filter(str_detect(NUTS2, 'DE')) |>
  mutate(year = make_date(year)) |> 
  ggplot(aes(x = year, y = gini_nuts2)) + 
  geom_line(aes(group = NUTS2), linewidth = 0.3) +
  #geom_smooth(mapping = aes(x = year, y = gini_nuts2)) +
  #geom_hline(yintercept = 0)
  ylim(0, 0.5)
```
...
Vi ser at for Frankrike er det en region (FR1) som har klart større forskjeller mht.
verdistgning enn de andre.
Sjekk denne regionen nærmere.
...
Det er to soner i NUTS3 som har mer enn dobbel så høy GDP per capita som de resterende.
Dette gjør Gini-koeffisienten høy.
...
Hent ut koeffisientene fra de 173 modellene og legg resultatet i variabelen mod_coeff.
Gjør dette ved å «mappe» funksjonen coeff() på list_column modell.
(Hint: Husk at hvis vi ønsker en dataframe så må `_df` varianten av map.)
...
For argumentet index kan dere bruke `index = c("NUTS2", "year")`.
...
Her forsøker vi å bruke plm(), men får feilmelding ang.
"not subsettable".

```{r}
# ag_comment: plm() ser ikke ut til å like list-columns, selv om vi
# ikke bruker dem. Plukker ut variablene vi trenger vha. select
p_mod <- NUTS2_diff_unnest |> 
  select(country_name:diff_gini_nuts2) %>%
  # data er nå i pipe. Skifter ovenfor til %>% og kan angi hvor data
  # skal inn vha: .
  plm('diff_gini_nuts2 ~ diff_gdp_per_capita', data = ., index = c("NUTS2", "year"))
```
